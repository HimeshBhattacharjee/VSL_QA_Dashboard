...
def fill_rot_test_data(worksheet, entries):
    try:
        ...
        log_progress(f"Error filling RoT test data: {str(e)}")
        raise
...
def fill_rot_signatures(worksheet, form_data):
    try:
        ...
        log_progress("RoT signatures filled successfully")
    except Exception as e:
        log_progress(f"Error filling RoT signatures: {str(e)}")
        raise
...
def generate_rot_report(rot_data):
    try:
        ...
        log_progress("Received RoT test data for report generation")
        log_progress(f"Report name: {rot_data.get('name', 'N/A')}")
        log_progress(f"Entries count: {len(rot_data.get('entries', []))}")
        log_progress(f"Form data keys: {list(rot_data.get('form_data', {}).keys())}")
        ...
        log_progress(f"RoT test report generated successfully: {filename}")
        return output, filename
    except Exception as e:
        log_progress(f"Error generating RoT test report: {str(e)}")
        raise   
