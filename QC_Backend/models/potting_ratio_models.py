...
potting_entries_collection.create_index([("date", ASCENDING), ("shift", ASCENDING)], unique=True)
...
class PottingDailyEntry:
    ...
    def create(entry_data: Dict[str, Any]) -> str:
        try:
            ...
            existing = potting_entries_collection.find_one({"date": entry_data["date"], "shift": shift})
    ...
    def get_by_date_and_shift(date: str, shift: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            return potting_entries_collection.find_one({"date": date_key, "shift": shift})
        except Exception as e:
    ...
    def delete_by_date_and_shift(date: str, shift: str) -> bool:
        try:
            result = potting_entries_collection.delete_one({"date": date, "shift": shift})
    ...
    def get_date_signatures(date: str) -> Optional[Dict[str, str]]:
        ...
        try:
            entries = potting_entries_collection.find({"date": date})
