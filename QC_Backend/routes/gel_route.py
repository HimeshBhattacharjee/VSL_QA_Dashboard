...
def load_gel_report_form_data(report: dict, report_payload: dict | None = None) -> dict:
    ...
    try:
        ...
        print(f"Warning: failed to load gel signature data for {report.get('_id')}: {exc}")
        return {}
...
def ensure_gel_report_metadata(report: dict, report_payload: dict | None = None) -> dict:
    ...
    try:
        ...
        print(f"Warning: failed to extract gel report metadata for {report.get('_id')}: {exc}")
        return existing_metadata
...
def backfill_missing_gel_metadata(limit: int = 200) -> None:
    ...
    try:
        ...
        print(f"Warning: failed to backfill gel report metadata: {exc}")
