...
def is_report_owner(report: dict, user: dict) -> bool:
    if report.get("autoGenerated") and not report.get("createdByEmployeeId"):
        return True
    return is_creator_match(report, user, (load_peel_report_form_data(report),))


def claim_automatic_draft(report: dict, user: dict) -> None:
    if not (report.get("autoGenerated") and not report.get("createdByEmployeeId")):
        return
    owner = {
        "createdBy": user["name"], "createdByUserId": user["id"],
        "createdByEmployeeName": user["name"], "createdByEmployeeId": user["employeeId"],
        "lockedBy": user["name"], "lockedByUserId": user["id"],
        "lockedByEmployeeId": user["employeeId"], "lockTimestamp": utc_timestamp(),
    }
    result = peel_test_collection.update_one(
        {"_id": report["_id"], "createdByEmployeeId": {"$in": [None, ""]}}, {"$set": owner}
    )
...
def extract_peel_report_metadata(form_data: dict, report: dict | None = None) -> dict:
    ...
    return {
        ...
        "wp": get_string_value(form_data, "wp") or report.get("wp") or "",
    }
...
def serialize_peel_report(report: dict, include_data: bool = False) -> dict:
    ...
    serialized = {
        ...
        "wp": metadata.get("wp", report.get("wp", "")),
        "autoGenerated": bool(report.get("autoGenerated")),
...
async def update_peel_test_report(report_id: str, report_data: dict, x_employee_id: str | None = Header(default=None)):
    try:
        ...
        claim_automatic_draft(existing_report, user)
...
async def submit_peel_test_report(report_id: str, report_data: dict | None = None, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        claim_result = peel_test_collection.update_one(
            {
                "_id": report_object_id,
                "workflowState": {"$in": list(EDITABLE_OPERATOR_STATES)},
                "automationSyncing": {"$ne": True},
                "submissionInProgress": {"$ne": True},
            },
            {"$set": {"submissionInProgress": True}},
        )
        if not claim_result.modified_count:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Report data is being synchronized; please retry submission")
        ...
        peel_test_collection.update_one(
            {"_id": report_object_id},
            {"$set": update_data, "$unset": {**{field: "" for field in LOCK_FIELDS}, "submissionInProgress": ""}},
        )
        submission_claimed = False
        ...
        if submission_claimed and report_object_id is not None:
            peel_test_collection.update_one({"_id": report_object_id}, {"$unset": {"submissionInProgress": ""}})
        raise
    except Exception as e:
        if submission_claimed and report_object_id is not None:
            peel_test_collection.update_one({"_id": report_object_id}, {"$unset": {"submissionInProgress": ""}})
