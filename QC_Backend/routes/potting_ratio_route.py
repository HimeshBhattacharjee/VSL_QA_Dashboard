...
def serialize_doc(doc):
    ...
    for line_num in ("1", "2"):
        ...
        line.setdefault("line", line_num)
        line.setdefault("po", "")
        line.setdefault("pottingSupplier", "")
        line.setdefault("partA", "")
    ...
    for line_num in ("1", "2"):
        ...
        normalized_lines[line_num] = normalize_line({
            **line,
            ...
            "pottingSupplier": _normalize_field_value(line.get("pottingSupplier")),
            "partA": _normalize_field_value(line.get("partA")),
            "partB": _normalize_field_value(line.get("partB")),
...
@potting_router.get("/dashboard")
async def get_potting_dashboard(
    ...
    try:
        ...
        return build_dashboard_response(
            collection=potting_entries_collection,
            query=query,
            view=view,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch potting ratio dashboard: {str(e)}")

@potting_router.get("/entries/register")
async def get_entry_register(
    ...
    try:
        ...
        total = potting_entries_collection.count_documents(query)
        entries = list(
...
@potting_router.get("/entries/by-id/{entry_id}")
async def get_entry_by_id(entry_id: str, x_employee_id: str | None = Header(default=None)):
    try:
        ...
        entry = PottingDailyEntry.get_by_id(entry_id)
        if not entry:
...
async def get_entries_for_date(date: str, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        date_signatures = PottingDailyEntry.get_context_signatures(date_key, "Line-I", "A")
        return {
            "success": True,
            "data": [serialize_entry(entry, user, include_permissions=bool(user)) for entry in entries],
            "date_signatures": date_signatures
...
@potting_router.get("/entries/{date}/{line_group}/{shift}")
async def get_entry_by_line_group(date: str, line_group: str, shift: str, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        if shift not in SHIFT_OPTIONS:
            raise HTTPException(status_code=400, detail="Shift must be A, B, or C")
        date_key = date.split('T')[0]
        entry = PottingDailyEntry.get_by_date_and_shift(date_key, shift, line_group)
        if entry:
...
async def get_entry(date: str, shift: str, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        date_key = date.split('T')[0]
        entry = PottingDailyEntry.get_by_date_and_shift(date_key, shift, "Line-I")
        if entry:
            return serialize_entry(entry, user, include_permissions=bool(user))
        return None
    except HTTPException:
...
async def create_entry(entry: dict, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        existing_entry = PottingDailyEntry.get_by_date_and_shift(
        ...
        PottingDailyEntry.create(update_data)
        saved_entry = PottingDailyEntry.get_by_date_and_shift(
            normalized_entry["date"],
            normalized_entry["shift"],
            normalized_entry["lineGroup"],
        )
...
@potting_router.post("/entries/{entry_id}/submit")
async def submit_entry(entry_id: str, entry: dict | None = None, x_employee_id: str | None = Header(default=None)):
    try:
        ...
        existing_entry = PottingDailyEntry.get_by_id(entry_id)
        ...
        updated = PottingDailyEntry.update_by_id(
            entry_id,
            update_data,
        ...
        submitted_entry = PottingDailyEntry.get_by_id(entry_id)
...
@potting_router.post("/entries/{entry_id}/approve")
async def approve_entry(entry_id: str, x_employee_id: str | None = Header(default=None)):
    try:
        ...
        approved_entry = PottingDailyEntry.get_by_id(entry_id)
...
@potting_router.post("/entries/{entry_id}/return")
async def return_entry(entry_id: str, request_data: dict, x_employee_id: str | None = Header(default=None)):
    try:
        ...
        returned_entry = PottingDailyEntry.get_by_id(entry_id)
...
@potting_router.post("/bulk/approve")
...
@potting_router.post("/bulk/delete")
        ...
        for raw_entry_id in entry_ids:
            ...
            try:
                ...
                if not can_delete_entry(existing_entry, user):
                    add_bulk_skip(result, get_bulk_status_label(existing_entry))
                    continue
                if not PottingDailyEntry.delete_by_id(entry_id):
...
async def update_signatures(payload: dict, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        existing_entry = PottingDailyEntry.get_by_date_and_shift(date, shift, line_group)
...
@potting_router.delete("/entries/by-id/{entry_id}")
async def delete_entry_by_id(entry_id: str, x_employee_id: str | None = Header(default=None)):
    try:
        ...
        deleted = PottingDailyEntry.delete_by_id(entry_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="Entry not found")
...
@potting_router.delete("/entries/{date}/{line_group}/{shift}")
async def delete_entry_by_line_group(date: str, line_group: str, shift: str, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        if not can_delete_entry(entry, user):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You are not authorized to delete this entry")
        deleted = PottingDailyEntry.delete_by_date_and_shift(date_key, shift, line_group)
        if not deleted:
            raise HTTPException(status_code=404, detail="Entry not found")
        return {"success": True, "message": "Entry deleted successfully"}
    except HTTPException:
...
async def delete_entry(date: str, shift: str, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        deleted = PottingDailyEntry.delete_by_date_and_shift(date_key, shift, "Line-I")
...
async def export_monthly_excel_post(payload: dict, x_employee_id: str | None = Header(default=None)):
    ...
    try:
        ...
        if user:
            for entry in payload.get("entries") or []:
                ...
                if entry_id and ObjectId.is_valid(str(entry_id)):
                    existing_entry = PottingDailyEntry.get_by_id(str(entry_id))
                if not existing_entry:
                    existing_entry = PottingDailyEntry.get_by_date_and_shift(
                        entry.get("date"),
                        entry.get("shift"),
        ...
        logger.exception("potting_excel_generation_failed")
