...
import { getUserInitials } from '../utilities/taskAssignments';
...
export default function MultiUserSelect({
    ...
    const handleRemoveUser = (user: string) => {
        onChange(selectedUsers.filter((selectedUser) => selectedUser !== user));
        inputRef.current?.focus();
    };
    ...
    return (
        <div ref={containerRef} className="relative">
            ...
            {isOpen && !disabled && (
                <div
                    id={`${inputId}-options`}
                    className="absolute z-20 mt-2 max-h-60 w-full overflow-y-auto rounded-2xl border border-slate-200 bg-white py-2 shadow-lg dark:border-slate-700 dark:bg-slate-900"
                >
                    {filteredOptions.length > 0 ? (
                        filteredOptions.map((option) => (
                            <button
                                ...
                                className="flex w-full items-center gap-3 px-3 py-2.5 text-left text-sm text-slate-700 transition-colors hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"
                            >
                                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-700 dark:bg-slate-700 dark:text-slate-100">
