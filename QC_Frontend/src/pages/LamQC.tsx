...
export default function LamQC() {
    ...
    const loadInitialData = async () => {
        try {
            const inspectionData = await loadInspectionData('lam-qc', 'combined');
            setLamQcDataset(inspectionData);
        } catch (error) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const inspectionData = await loadInspectionData('lam-qc', selectedLine);

            // Immediately update state so UI remains consistent
            setLamQcDataset(prev => ({
                ...prev,
                'lam-qc': inspectionData
            }));
            ...
            console.error('Error fetching Lam-QC data:', error);
    ...
    const chartOptions = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: `Defect Analysis - Lam QC ${selectedLine === 'combined' ? '(All Lines)' : `(Line ${selectedLine})`}`,
                font: {
                    size: 16
                }
            }
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <div className="display bg-white dark:bg-gray-900 rounded-2xl p-4 sm:p-5 md:p-6 shadow-2xl transition-colors duration-300">
                <div className="date-selector flex flex-col md:flex-row gap-3 sm:gap-4 items-center justify-center flex-wrap">
