...
export default function RoTTest() {
    ...
    const [savedReports, setSavedReports] = useState<RoTTestReport[]>([]);
    ...
    const apiService = {
        getAllReports: async (): Promise<RoTTestReport[]> => {
            const response = await fetch(`${ROT_API_BASE_URL}`);
            if (!response.ok) {
        ...
        getReportById: async (id: string): Promise<RoTTestReport> => {
            const response = await fetch(`${ROT_API_BASE_URL}/${id}`);
            if (!response.ok) {
                const errorText = await response.text();
        ...
        createReport: async (report: Omit<RoTTestReport, '_id'>): Promise<RoTTestReport> => {
            const response = await fetch(`${ROT_API_BASE_URL}`, {
                method: 'POST',
        ...
        updateReport: async (id: string, report: Omit<RoTTestReport, '_id'>): Promise<RoTTestReport> => {
            const response = await fetch(`${ROT_API_BASE_URL}/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(report),
        ...
        deleteReport: async (id: string): Promise<void> => {
            const response = await fetch(`${ROT_API_BASE_URL}/${id}`, { method: 'DELETE' });
            if (!response.ok) {
                const errorText = await response.text();
        ...
        checkReportNameExists: async (name: string, excludeId?: string): Promise<boolean> => {
            const url = `${ROT_API_BASE_URL}/name/${encodeURIComponent(name)}${excludeId ? `?excludeId=${excludeId}` : ''}`;
    ...
    const loadReportData = (report: RoTTestReport) => {
    ...
    useEffect(() => {
        if (activeTab === 'edit-report') {
            ...
            if (editingReportData) {
                clearFormData(false);
                setTimeout(() => {
                    const report = JSON.parse(editingReportData) as RoTTestReport;
    ...
    const saveFormData = () => {
        ...
        sessionStorage.setItem('rotTestFormData', JSON.stringify(formData));
    };

    const loadFormData = () => {
        const savedData = sessionStorage.getItem('rotTestFormData');
        if (savedData) {
    ...
    const clearFormData = (clearEditingState = true) => {
        ...
        if (clearEditingState) {
            sessionStorage.removeItem('rotTestFormData');
        }
    ...
    const saveReport = async () => {
        ...
        try {
            ...
            const reportData: Omit<RoTTestReport, '_id'> = {
                name: reportName,
    ...
    const exportToExcel = async () => {
        try {
            ...
            const rotReportData = {
                report_name: reportName.trim() || 'ROT_Test_Report',
                timestamp: new Date().toISOString(),
                form_data: formData,
            };
            ...
            a.href = url;
            a.download = `${reportName.trim() || 'ROT_Test_Report'}.xlsx`;
    ...
    const renderDataRows = () => {
        ...
        for (let i = 0; i < 30; i++) {
            rows.push(
                <tr key={i}>
                    <td className="editable data-cell h-5 min-w-5 cursor-text relative border border-transparent transition-border-color duration-200 ease-in-out dark:text-white hover:border-blue-500 dark:hover:border-blue-400 p-2 text-center"></td>
                    <td className="editable data-cell h-5 min-w-5 cursor-text relative border border-transparent transition-border-color duration-200 ease-in-out dark:text-white hover:border-blue-500 dark:hover:border-blue-400 p-2 text-center"></td>
                    <td colSpan={2} className="editable data-cell h-5 min-w-5 cursor-text relative border border-transparent transition-border-color duration-200 ease-in-out dark:text-white hover:border-blue-500 dark:hover:border-blue-400 p-2 text-center"></td>
                    <td colSpan={2} className="editable data-cell h-5 min-w-5 cursor-text relative border border-transparent transition-border-color duration-200 ease-in-out dark:text-white hover:border-blue-500 dark:hover:border-blue-400 p-2 text-center"></td>
                    <td className="editable data-cell h-5 min-w-5 cursor-text relative border border-transparent transition-border-color duration-200 ease-in-out dark:text-white hover:border-blue-500 dark:hover:border-blue-400 p-2 text-center"></td>
                    <td className="editable data-cell h-5 min-w-5 cursor-text relative border border-transparent transition-border-color duration-200 ease-in-out dark:text-white hover:border-blue-500 dark:hover:border-blue-400 p-2 text-center"></td>
                    <td colSpan={2} className="editable data-cell h-5 min-w-5 cursor-text relative border border-transparent transition-border-color duration-200 ease-in-out dark:text-white hover:border-blue-500 dark:hover:border-blue-400 p-2 text-center"></td>
                                ...
                                <table ref={tableRef} className="w-full border-collapse min-w-[800px]">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td colSpan={6} className="section-title text-xl sm:text-2xl md:text-3xl font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">
                                            ...
                                            <td colSpan={6} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-sm sm:text-base text-gray-800 dark:text-white">
                                                Doc. No.: VSL/QAD/FM/124
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colSpan={6} className="section-title text-lg sm:text-xl md:text-2xl font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">
                                                Robustness of Terminations Test Report
                                            </td>
                                            <td colSpan={6} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-sm sm:text-base text-gray-800 dark:text-white">
                                        ...
                                        <tr>
                                            <td colSpan={6}>
                                                <div className="p-2.5 bg-gray-50 dark:bg-gray-900 border-l-4 border-l-blue-500 dark:border-l-blue-400 text-left">
                                                    <strong className="px-2.5 text-gray-800 dark:text-white">Specifications:</strong>
                                                    <div className="flex flex-col sm:flex-row gap-2 mt-2 mx-2 text-sm text-gray-700 dark:text-gray-300">
                                                        1) Get a Weight of 5Kgs (40N) in case of IEC and 15 Kgs (156 N) in case of UL for all types of module.
                                                    </div>
                                                    <div className="flex flex-col sm:flex-row gap-2 mt-2 mx-2 text-sm text-gray-700 dark:text-gray-300">
                                                        2) Tie the cables of the JB with the weight and hang it for 1 minutes in 5 different directions.
                                                    </div>
                                                </div>
                                            </td>
                                            <td colSpan={6} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-sm sm:text-base text-gray-800 dark:text-white">
                                        ...
                                        <tr>
                                            <td colSpan={14} className="passing-criteria p-3 bg-gray-50 dark:bg-gray-900 border-l-4 border-l-green-500 dark:border-l-green-400">
                                                <div className="text-left text-gray-800 dark:text-white">
                                                    <strong>Passing Criteria:</strong>
                                                    <span className="ml-2 text-gray-700 dark:text-gray-300">
                                                        JB should not be detached from back sheet/ Rear glass.
                                                    </span>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td colSpan={2} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">Module Type</td>
                                            <td colSpan={2} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">Module Serial no.</td>
                                            ...
                                            <td className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">Sealant Supplier</td>
                                            <td colSpan={2} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">Back sheet/Rear Glass Supplier</td>
                                            <td colSpan={2} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">Result</td>
                                            <td colSpan={2} className="section-title font-bold bg-gray-100 dark:bg-gray-700 text-center p-2 text-gray-800 dark:text-white">Test Done By</td>
                                        </tr>
                                        {renderDataRows()}
                                    </tbody>
                                </table>
