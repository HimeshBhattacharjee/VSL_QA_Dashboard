...
const getResultInputClass = (value?: string | number) => {
    ...
    return 'bg-gray-50 text-gray-900 border-gray-200 dark:bg-gray-800 dark:text-gray-200 dark:border-gray-700';
};
...
const normalizeEntry = (entry: DailyEntry): DailyEntry => {
    ...
    return {
        ...
        signatures: {
            ...
            approvedBy: entry.signatures?.approvedBy || '',
        },
    };
};
...
const hasAnySSHData = (entry?: DailyEntry) => {
...
const getEntryValidationMessage = (entry: DailyEntry): string | null => {
    ...
    const requiredLineFields: Array<{ key: keyof LineEntry; label: string }> = [
    ...
    const lineRequiredDetails = (line: LineEntry, lineNumber: '1' | '2') =>
        requiredLineFields.map(({ key, label }) => ({
...
export default function SSHTest() {
    ...
    const API_BASE_URL = `${import.meta.env.VITE_API_URL}/ssh-test-reports`;
    ...
    const loadMonthlyData = useCallback(async (year: number, month: number) => {
        ...
        try {
            ...
            entriesArr.forEach(entry => {
                ...
                const signature = nextSignatures[entryKey] || entry.signatures || { preparedBy: '', approvedBy: '' };
    ...
    const loadDashboard = useCallback(async () => {
        try {
            ...
            console.error('Error loading SSH dashboard:', error);
    ...
    const handleSignatureUpdate = async (type: 'prepared' | 'approved') => {
        if (!username) {
        ...
        const field = type === 'prepared' ? 'preparedBy' : 'approvedBy';
        const signatureKey = getEntryKey(currentEntry.date, currentEntry.lineGroup, currentEntry.shift);
        const currentSignatures = dateSignatures[signatureKey] || currentEntry.signatures || { preparedBy: '', approvedBy: '' };
        ...
        if (nextSignatures[field]) {
            ...
            if (type === 'approved' && !['Manager', 'Supervisor'].includes(userRole || '')) {
                showAlert('error', 'Only Managers or Supervisors can sign as Approved By');
                return;
            }
    ...
    const exportMonthlyExcelFor = async (year: number, month: number, lineGroup: LineGroup) => {
        ...
        try {
            ...
            const exportLineNumbers = getDisplayLineNumbers(lineGroup);
            const entries = (Array.isArray(monthlyJson?.data) ? monthlyJson.data.map(normalizeEntry) : [])
                ...
                .map((entry: DailyEntry) => ({
                    ...
                    lines: {
                        ...entry.lines,
                        '1': { ...entry.lines['1'], line: exportLineNumbers[0] },
                        '2': { ...entry.lines['2'], line: exportLineNumbers[1] },
                    },
                }));
            if (entries.length === 0) {
            ...
            anchor.href = url;
            anchor.download = `${reportName}.xlsx`;
    ...
    const renderCalendarDays = () => {
        ...
        for (let day = 1; day <= daysInMonth; day += 1) {
            ...
            const hasAny = entriesForDate
                .some(entry => Boolean(entry) && (hasAnySSHData(entry) || entry?.signatures?.preparedBy || entry?.signatures?.approvedBy));
        ...
        return (
            <div className="mt-6 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        <label className="mb-2 block text-xs font-medium text-gray-700 dark:text-gray-300">Approved By</label>
                        <div className="flex items-center gap-2">
                            <input
                                type="text"
            ...
            <div className={`border-l-4 ${accentClass} pl-4 ${isFirstLine ? '' : 'mt-6'}`}>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <div>
                        ...
                        <select
                            value={currentEntry.lines[lineKey].sealantSupplier}
                            onChange={(event) => handleLineInputChange(lineKey, 'sealantSupplier', event.target.value)}
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Sealant Expiry Date</label>
                        <input
                            type="text"
                            value={currentEntry.lines[lineKey].sealantExpDate}
                            onChange={(event) => handleLineInputChange(lineKey, 'sealantExpDate', event.target.value)}
                            disabled={!canEditCurrentEntry}
                            className="w-full rounded-lg border border-gray-200 bg-gray-50 p-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary disabled:cursor-not-allowed disabled:opacity-80 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Sample Taking Time</label>
                        <input
                            type="text"
                            value={currentEntry.lines[lineKey].sampleTakingTime}
                            onChange={(event) => handleLineInputChange(lineKey, 'sampleTakingTime', event.target.value)}
                            disabled={!canEditCurrentEntry}
                            className="w-full rounded-lg border border-gray-200 bg-gray-50 p-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary disabled:cursor-not-allowed disabled:opacity-80 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                            placeholder="HH:MM"
                        />
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Sample Testing Time</label>
                        <input
                            type="text"
                            value={currentEntry.lines[lineKey].sampleTestingTime}
                            onChange={(event) => handleLineInputChange(lineKey, 'sampleTestingTime', event.target.value)}
                            disabled={!canEditCurrentEntry}
                            className="w-full rounded-lg border border-gray-200 bg-gray-50 p-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary disabled:cursor-not-allowed disabled:opacity-80 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                            placeholder="HH:MM"
                        />
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Result (Shore A)</label>
                        <input
                            type="number"
                            value={getResultInputValue(currentEntry.lines[lineKey].result)}
                            onChange={(event) => handleLineInputChange(lineKey, 'result', event.target.value)}
                            disabled={!canEditCurrentEntry}
                            className={`w-full rounded-lg border p-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary disabled:cursor-not-allowed disabled:opacity-80 ${getResultInputClass(currentEntry.lines[lineKey].result)}`}
    ...
    return (
        <div className="mx-auto">
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    {SHIFTS.map(shift => {
                                        ...
                                        const line1Result = entry?.lines['1']?.result;
                                        const line2Result = entry?.lines['2']?.result;
                                        return (
                                            <button
                                                key={shift}
                                                ...
                                                <div className="grow text-left">
                                                    ...
                                                    {isFilled ? (
                                                        <div className="text-sm text-gray-600 dark:text-gray-400">
                                                            <div>PO: {entry?.po || '-'}</div>
                                                            <div>Line {getDisplayLineNumbers(shiftSelectorLineGroup)[0]} Result: {line1Result || '-'}</div>
                                                            <div>Line {getDisplayLineNumbers(shiftSelectorLineGroup)[1]} Result: {line2Result || '-'}</div>
                                                        </div>
                                                    ) : (
            ...
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Sealant Shore Hardness Test</h1>
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Acceptance criteria: &gt;= 39 Shore A</p>
                </div>
                        ...
                        {currentEntry && selectedShift && (
                            <div className="rounded-xl bg-white p-6 shadow-lg dark:bg-gray-900">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        <div>
                                            <label className="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Checked By</label>
                                            <input
                                                type="text"
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(event) => handleInputChange('po', event.target.value)}
                                                disabled={!canEditCurrentEntry}
                                                className="w-full rounded-lg border border-gray-200 bg-gray-50 p-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary disabled:cursor-not-allowed disabled:opacity-80 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                                    ...
                                    return (
                                        <div key={shift} className="rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            <div className="mt-2 flex flex-wrap gap-3 text-xs">
                                                <span className="text-green-600 dark:text-green-300">Pass: {stats.pass}</span>
                                                <span className="text-red-600 dark:text-red-300">Fail: {stats.fail}</span>
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        <div className="rounded-xl bg-white p-6 shadow-lg dark:bg-gray-900">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="rounded-md border border-red-200 bg-red-50 p-3 dark:border-red-700 dark:bg-red-900/20">
                                    <div className="text-xs text-red-700 dark:text-red-300">Fail (&lt; 39 Shore A)</div>
                                ...
                                <div className="mt-4 rounded-md border border-gray-200 bg-gray-50 p-3 text-xs dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {(['1', '2'] as const).map(lineKey => {
                                            const displayLine = getDisplayLineNumbers(currentEntry.lineGroup)[lineKey === '1' ? 0 : 1];
                                            const result = currentEntry.lines[lineKey].result;
                                            return (
                                                <span key={lineKey} className={isSSHResultPass(result) ? 'text-green-600 dark:text-green-300' : isSSHResultFail(result) ? 'text-red-600 dark:text-red-300' : 'text-gray-500 dark:text-gray-400'}>
                                                    Line {displayLine}: {result || '-'}
                                                </span>
                                            );
                                        })}
                                    </div>
