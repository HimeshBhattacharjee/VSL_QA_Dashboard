...
export default function Visual() {
    ...
    const loadInitialData = async () => {
        try {
            const inspectionData = await loadInspectionData('visual', 'combined');
            setVisualDataset(inspectionData);
        } catch (error) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const inspectionData = await loadInspectionData('visual', selectedLine);

            // Immediately update state so UI remains consistent
            setVisualDataset(prev => ({
                ...prev,
                'visual': inspectionData
            }));
    ...
    const chartOptions = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: `Defect Analysis - Visual ${selectedLine === 'combined' ? '(All Lines)' : `(Line ${selectedLine})`}`,
                font: {
                    size: 16
                }
            }
